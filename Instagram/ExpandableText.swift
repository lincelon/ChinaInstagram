...
struct ExpandableText: View {
    ...
    init(username: String, text: String, lineLimit: Int, font: UIFont = UIFont.preferredFont(forTextStyle: UIFont.TextStyle.body)) {
                ...
                Text(username + text)
                    ...
                    .background(GeometryReader { visibleTextGeometry in
                        Color.clear.onAppear() {
                            ...
                            var low  = 0
                            var heigh = username.count + " ".count + shrinkText.count
                            var mid = heigh ///start from top so that if text contain we does not need to loop
                            while ((heigh - low) > 1) {
                                let attributedText = NSAttributedString(string:  username + " " + shrinkText + moreLessText, attributes: attributes)
